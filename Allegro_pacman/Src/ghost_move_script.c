...
static Directions counter_one(Directions dir) {
	switch (dir) {
	case UP:
		return DOWN;
	case DOWN:
...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	if (cnt >= 1) {
		// Choose a random direction from proba array
	...
	*/
	...
	Directions nextMove = proba[rand() % cnt];

	if (ghost->objData.preMove == counter_one(nextMove)) {
		while (nextMove == counter_one(ghost->objData.preMove)) {
			nextMove = proba[rand() % cnt];
		}
	}

	ghost_NextMove(ghost, nextMove);
}
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++) {
		if (ghost_movable(ghost, M, i, true) && i != shortestDirection && i != counter_one(ghost->objData.preMove)) {
			proba[cnt++] = i;
		}
	}
	...
	if (cnt == 0) nextMove = shortestDirection;
	else nextMove = proba[rand() % cnt];
